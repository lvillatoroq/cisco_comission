...
overall_log_file = 'files/feedback_hirschmann_RSP.txt'
...
def ssh_to_sw (device,output_main):
    try:
        ...
        # output_dict[device]['hostname'] = device_name[:-1]
        with open(outputFileName, 'wb') as f:
            #set terminal length to 0 
            net_connect.send_command('cli numlines 0')
            #Run commands extracted from excel file and write to output file
            for command in commands:
                f.write('###########################\r\n')
                f.write(command + '\r\n')
                f.write('###########################\r\n')
                #Use send command timing only for commands that take long to execute, also use it for hirschamann switches
                output = net_connect.send_command_timing(command,delay_factor=4,max_loops=100)
                f.write(output + '\r\n\r\n')
                if output in invalid_command:
...
logging.basicConfig(filename='files/ssh_logs_hirschmann.txt', level=logging.DEBUG)
